require 'spec_helper'

describe "content_areas/edit.html.haml" do
  before(:each) do
    @content_area = assign(:content_area, stub_model(ContentArea,
      :name => "MyString",
      :content => "MyText",
      :sort_order => 1
    ))
  end

  it "renders the edit content_area form" do
    render

    # Run the generator again with the --webrat flag if you want to use webrat matchers
    assert_select "form", :action => content_areas_path(@content_area), :method => "post" do
      assert_select "input#content_area_name", :name => "content_area[name]"
      assert_select "textarea#content_area_content", :name => "content_area[content]"
      assert_select "input#content_area_sort_order", :name => "content_area[sort_order]"
    end
  end
end
